...
用户可查看 `API 说明 <./ApiReference.rst>`__ 获取 Trainer 的详细用法。 "`write a trainer <./WriteTrainer.rst>`__" for how to write a new trainer, and refer to `this document <./WriteStrategy.rst>`__ for how to write a new strategy.

发起 Experiment
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

上述内容准备就绪之后，就可以发起 Experiment 以进行模型搜索了。 We design unified interface for users to start their experiment. An example is shown below
